function extLoadsFileName = createExternalLoadsProsthesis(devices, sides, extLoadsFileName, tf_type)
...
for i = 1:length(devices) % add each device as an external force
    objs.appendChild(createExternalForce(devices{i}, sides(i), tf_type));
end
...
% name given in device, that acts on the side given by side
    function extForce = createExternalForce(device, side, tf_type)
        extForce = dom.createElement('ExternalForce');
        extForce.setAttribute('name', [device '_extForce']);
        if ~strcmp(side, tf_type(2))
            addChild(extForce, 'applied_to_body', ['calcn_' side]);
        elseif strcmpi(tf_type(1), 'e')
            addChild(extForce, 'applied_to_body', 'ARJoint');
        elseif strcmpi(tf_type(1), 'p') || strcmpi(tf_type(1), 'o')
            addChild(extForce, 'applied_to_body', 'Foot');
            % addChild(extForce, 'applied_to_body', ['calcn_' side]);
        end
