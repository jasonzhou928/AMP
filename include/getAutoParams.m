function [clusters_jump_threshold] = getAutoParams(clusters, markerStructRef, folderPath, trialList, viconPath,paramStruct, subjects, useVicon)
...
%% Get trial subset
trialNum = floor(length(trialList)*.15)+1;
if trialNum > 5
    trialNum = 5;
end
...
for i = trialInds
    ...
    for ci = 1:length(clusters)
        ...
        previous_jump_thresholds{ci} = {NaN};
        lower_bound_thresholds{ci} = {NaN};
        enforce_high_thresholds{ci} = NaN;
    end
    ...
    for cc = 1:length(clusters)
        ...
        if previous_jump_thresholds{cc}{1} < 4
            thresholdArr(cc,currentTrialInd) = 4; %min jump thresh
        else    
