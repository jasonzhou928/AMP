function processTrialsNoVicon(clusters,clusters_jump_threshold, markerStructRef, folderPath, trialList,paramStruct, subjects, useVicon, app)
...
for i = 1:length(trialList)
    ...
    try
    copyfile([folderPath '\' trialList{i} '.trial.enf'],[folderPath '\Working\' trialList{i} '.trial.enf'])
    endNoteExists = true;
    catch
        endNoteExists = false;
    end
    ...
    if labelingIssue
         ...
         warning('Problem labeling trial')
...
%         [starting,ending] = Find_First_And_Last_Frames(WBAM_Markerset, markerStruct, markerStructRef, clusters);
        ...
        try
            ...
            while filled
                ...
                [markerStruct, filled] = Rigid_Body_Fill_All_Gaps(markerSet, markerStruct, clusters, verbose);
            end
    %         missingFilled = Vicon.findGaps(markerStruct);
            ...
            warning('Problem Gap Filling Trial')
            copyfile([folderPath '\Working\' trialList{i} '_preprocessed.c3d'],[folderPath '\Failed\' trialList{i} '.c3d'])
            delete([folderPath '\Working\' trialList{i} '_preprocessed.c3d'])
            delete([folderPath '\Working\' trialList{i} '.c3d'])
            if endNoteExists
                copyfile([folderPath '\Working\' trialList{i} '.trial.enf'],[folderPath '\Failed\' trialList{i} '.trial.enf']) 
                delete([folderPath '\Working\' trialList{i} '.trial.enf'])
